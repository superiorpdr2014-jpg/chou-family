...
const LABELS = {
  ...
  /*
   * 周炳辰（老二）：1312 = 二房辦聚會_15、4557 = 壽宴_13
   * 「周阿扁」是綽號，本名周炳辰 —— 這是比對自己解出來的：
   * Jay 先說老二叫「周阿扁」，後來給壽宴_13 說右二是「周炳辰」，
   * 而那張臉比對到周阿扁 0.334 ✅ —— 同一個人、同樣是老二/右二。
   */
  'zhou-bingchen': [1312, 4557],
  ...
  /*
   * 壽宴 _13：周銘陽全家。Jay 口述「最右邊是周奕辰，右二是周炳辰，周銘陽旁邊是周銘陽老婆」
   * 錨點：周炳辰(原周阿扁) 0.334 ✅、周銘陽 0.369、周筠庭 0.396
   */
  'zhou-mingyang-wife': [4558],       // 周銘陽的老婆：周銘陽旁邊

  /*
   * 壽宴 _20：Jay 說「周曉妤在這邊」。照片只有兩個人，女生(#2)就是她 ——
   * 她之前在 _67 笑到仰頭、臉糊掉(信心 0.496)不能用，這張 174px 終於能標。
   */
  'zhou-xiaoyu': [4599],

  /*
   * 20161001 慶生 _45：Jay 說「我的二阿姨跟他老公」，周月梅 0.358 定位。
   * ⚠️ 前景這兩張臉都 190px+ 卻只有 0.33/0.42 信心 —— 又是「離鏡頭太近導致信心低」
   *    的誤判（臉本身很清楚），照用。
   */
  'zhou-yuemei-husband': [2907],      // 二阿姨的老公：條紋 polo

  /*
   * 婚宴照 _15：Jay 說「馬成新的老婆蘇佩茹，以及女兒馬惟薇」
   * 錨點：馬惟薇 0.260 ✅ 在 #0，所以 #1 是蘇佩茹。
   * 同時把馬肇均/馬惟薇的父母補上媽媽（原本只掛馬成新，樹上會像單親）。
   */
  'su-peiru': [327],                  // 蘇佩茹：右邊，米白毛衣

  /*
   * 壽宴 _8：Jay 說「周東興跟何碧嬿」—— 兩個都驗證通過
   * （周東興 0.310 ✅、何碧嬿 0.320 ✅），純粹補一張好照片
   */

  /*
   * 壽宴 _9：Jay 說「右下角就是王法成(程?)，四阿姨的大兒子，上面三個小女生都是他的女兒」
   * 同張 王法正 0.379（他弟弟）也在，合理。
   * ⚠️ 名字待確認：Jay 先前給「王法程」，這次寫「王法成」。
   */
  'wang-facheng': [4538],             // 四阿姨的大兒子：右下角、粉紅 polo 戴眼鏡
